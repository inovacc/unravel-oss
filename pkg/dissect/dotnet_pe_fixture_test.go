...
// synthManagedAssemblyForDissect writes a minimal-but-valid managed PE to path:
// one type Demo.Greeter with a single method Hello() whose body is
// `ldstr "hi"; call [System.Console]System.Console::WriteLine(string); ret`.
//
// It is a dissect-package twin of decompile.synthManagedAssembly (clrgen-backed
// byte layout). The native engine's clr.Open(path) -> clr.ExtractModules path
// yields >=1 type module from this fixture, which is exactly what the
// silent-empty regression guard (TestDotNetCapture_NativeNonEmpty) asserts.
//
// When the shared Testing-group clrgen emitter lands, collapse this onto it.
func synthManagedAssemblyForDissect(t *testing.T, path string) {
	t.Helper()
	if err := os.WriteFile(path, dnfxBuildGreeterPE(), 0o644); err != nil {
		t.Fatalf("write managed assembly %s: %v", path, err)
	}
	// Sibling *.deps.json so the FullApp walk can discover the assembly by base
	// name. Harmless for the ModeSingle path used by the regression guard.
...
func (b *dnfxMeta) intern(s string) uint16 {
...
func (b *dnfxMeta) addUserString(s string) uint16 {
	off := uint16(len(b.us))
...
func (b *dnfxMeta) addBlob(data []byte) uint16 {
...
func dnfxU16(v uint16) []byte { return []byte{byte(v), byte(v >> 8)} }
func dnfxU32(v uint32) []byte {
	return []byte{byte(v), byte(v >> 8), byte(v >> 16), byte(v >> 24)}
}

func (b *dnfxMeta) appendRow(id byte, row []byte) {
	b.rows[id]++
	b.rowData[id] = append(b.rowData[id], row...)
}

func (b *dnfxMeta) emitAssembly(name, culture uint16) {
	row := dnfxU32(0)
	for i := 0; i < 4; i++ {
		row = append(row, dnfxU16(0)...)
	}
	row = append(row, dnfxU32(0)...) // Flags
	row = append(row, dnfxU16(0)...) // PublicKey blob idx
	row = append(row, dnfxU16(name)...)
	row = append(row, dnfxU16(culture)...)
	b.appendRow(0x20, row)
}

func (b *dnfxMeta) emitAssemblyRef(name uint16) {
	row := []byte{}
	for i := 0; i < 4; i++ {
		row = append(row, dnfxU16(0)...)
	}
	row = append(row, dnfxU32(0)...) // Flags
	row = append(row, dnfxU16(0)...) // PublicKeyOrToken blob idx
	row = append(row, dnfxU16(name)...)
	row = append(row, dnfxU16(0)...) // Culture
	row = append(row, dnfxU16(0)...) // HashValue blob idx
	b.appendRow(0x23, row)
}

func (b *dnfxMeta) emitTypeRef(resolutionScope, name, namespace uint16) {
	row := dnfxU16(resolutionScope)
	row = append(row, dnfxU16(name)...)
	row = append(row, dnfxU16(namespace)...)
	b.appendRow(0x01, row)
}

func (b *dnfxMeta) emitMemberRef(class, name, sig uint16) {
	row := dnfxU16(class)
	row = append(row, dnfxU16(name)...)
	row = append(row, dnfxU16(sig)...)
	b.appendRow(0x0A, row)
}

func (b *dnfxMeta) emitTypeDef(name, namespace, extends, fieldStart, methodStart uint16) {
	row := dnfxU32(0)
	row = append(row, dnfxU16(name)...)
	row = append(row, dnfxU16(namespace)...)
	row = append(row, dnfxU16(extends)...)
	row = append(row, dnfxU16(fieldStart)...)
	row = append(row, dnfxU16(methodStart)...)
	b.appendRow(0x02, row)
}

func (b *dnfxMeta) emitMethodDef(rva uint32, name, sig, paramList uint16) {
	row := dnfxU32(rva)
	row = append(row, dnfxU16(0)...) // ImplFlags = 0 (managed IL)
	row = append(row, dnfxU16(0)...) // Flags
	row = append(row, dnfxU16(name)...)
	row = append(row, dnfxU16(sig)...)
	row = append(row, dnfxU16(paramList)...)
	b.appendRow(0x06, row)
}

func dnfxMemberRefParentTypeRef(rid uint16) uint16 { return rid<<3 | 1 }

func dnfxAlign4(n int) int { return (n + 3) &^ 3 }

const dnfxBSJB = 0x424A5342

func (b *dnfxMeta) build() []byte {
	tilde := b.buildTilde()
	...
	for _, s := range streams {
		nm := s.name + "\x00"
		headerLen += 8 + dnfxAlign4(len(nm))
	}

	out := make([]byte, headerLen)
	binary.LittleEndian.PutUint32(out[0:], dnfxBSJB)
	...
	for _, s := range streams {
		...
		nm := s.name + "\x00"
		copy(out[p+8:], nm)
		p += 8 + dnfxAlign4(len(nm))
		bodies = append(bodies, s.data...)
	}
	return append(out, bodies...)
}

func (b *dnfxMeta) buildTilde() []byte {
	...
	for id := byte(0); id <= maxTableID; id++ {
		if valid&(1<<id) != 0 {
			out = append(out, dnfxU32(b.rows[id])...)
		}
	}
	for id := byte(0); id <= maxTableID; id++ {
		if valid&(1<<id) != 0 {
...
func dnfxRoundUp(n, a int) int { return (n + a - 1) / a * a }

// dnfxBuildGreeterPE returns the raw bytes of a one-type managed PE.
func dnfxBuildGreeterPE() []byte {
	const (
		ntHeaderOffset = 0x80
		secVA          = 0x2000
		secRaw         = 0x400
	...
	m.emitMemberRef(dnfxMemberRefParentTypeRef(1), m.intern("WriteLine"), writeLineSig)
	...
	binary.LittleEndian.PutUint32(sh[16:], uint32(dnfxRoundUp(rawLen, 0x200)))
