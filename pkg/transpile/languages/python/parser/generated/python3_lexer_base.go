...
func (l *Python3LexerBase) NextToken() antlr.Token {
	...
	// Return queued tokens first (synthetic NEWLINE/INDENT/DEDENT from onNewLine)
	if len(l.tokens) > 0 {
		x := l.tokens[0]
		l.tokens = l.tokens[1:]
	...
	// If actions during BaseLexer.NextToken() queued new tokens,
	// return from queue front. Only append next if it's not already
	// the last queued token (EmitToken sets both queue and l.token,
	// but Skip() causes a new token to be matched instead).
	if len(l.tokens) > 0 {
		if l.tokens[len(l.tokens)-1] != next {
			l.tokens = append(l.tokens, next)
		}
		x := l.tokens[0]
		l.tokens = l.tokens[1:]
		if x.GetChannel() == antlr.TokenDefaultChannel {
